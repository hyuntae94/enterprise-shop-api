...
async def create_order(data: OrderCreate, user: CurrentUser, service: OrderServiceDep) -> OrderRead:
...
async def list_my_orders(
    ...
    items, total = await repo.list_for_user(user.id, offset=params.offset, limit=params.size)
...
async def cancel_order(order_id: int, user: CurrentUser, service: OrderServiceDep) -> OrderRead:
